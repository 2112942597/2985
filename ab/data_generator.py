...
def getData(X_shape, flag="test"):
    ...
    if flag == "MC":
        ...
        for i in (MC_files):
            im = cv2.resize(cv2.imread(os.path.join(MC_path, i)), (X_shape, X_shape))  # [:,:,0]
            ma_ = Image.open(os.path.join(MC_path_mask, i))

            ma = ma_.resize((256, 256), Image.ANTIALIAS)
            ma = np.array(ma)
            ma[ma > 0] = 255

            augmented = AUGMENTATIONS_TEST(image=im, mask=ma)
            image_padded = augmented['image']
            mask_padded = augmented['mask']
            im_array.append(image_padded)
            mask_array.append(mask_padded / 255.0)
    ...
    if flag == "JSRT":
        for i in (JSRT_files_):
            ...
            mask_padded = augmented['mask']
            im_array.append(image_padded)
            mask_array.append(mask_padded / 255.0)  # /255.0
    ...
    x_data = HAUT[:1000]  # X_test+JSRT+
    y_data = HAUT_MASK[:1000]  # y_test+JSRT_MASK+
